...
assert.equal(selectManualProcessView(manualRewardEngine.state, ruleset, 'MANUAL_PRIMORDIAL_PULSE').cooldownMs, 3500);
...
manualRewardEngine.tick(3499);
assert.equal(selectManualProcessView(manualRewardEngine.state, ruleset, 'MANUAL_PRIMORDIAL_PULSE').available, false);
manualRewardEngine.tick(1);
assert.equal(selectManualProcessView(manualRewardEngine.state, ruleset, 'MANUAL_PRIMORDIAL_PULSE').available, true);
manualRewardEngine.dispatch({ type: 'ADD_RESOURCE', resourceId: 'energy', amount: 40 });
manualRewardEngine.dispatch({ type: 'ADD_RESOURCE', resourceId: 'information', amount: 3 });
result = manualRewardEngine.dispatch({ type: 'BUY_NODE', nodeId: 'M02' });
assert.equal(result.ok, true);
assert.equal(selectManualProcessView(manualRewardEngine.state, ruleset, 'MANUAL_PRIMORDIAL_PULSE').cooldownMs, 90000);
result = manualRewardEngine.dispatch({ type: 'USE_MANUAL_PROCESS', processId: 'MANUAL_PRIMORDIAL_PULSE' });
assert.equal(result.ok, true);
manualRewardEngine.tick(89999);
assert.equal(selectManualProcessView(manualRewardEngine.state, ruleset, 'MANUAL_PRIMORDIAL_PULSE').available, false);
manualRewardEngine.tick(1);
assert.equal(selectManualProcessView(manualRewardEngine.state, ruleset, 'MANUAL_PRIMORDIAL_PULSE').available, true);
