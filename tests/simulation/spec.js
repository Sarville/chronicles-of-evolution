...
between(competent.timings.selfReplicationAtMs, 110000, 150000, 'M02');
between(competent.timings.catalyticRnaAtMs, 220000, 300000, 'M03');
...
between(optimized.timings.protoCellAtMs, 450000, 660000, 'optimized scripted M06');

const slow = runHeadlessSimulation({ seed: 7, profile: 'slow' });
assert.equal(slow.ok, true);
assert.equal(slow.timings.protoCellAtMs <= 780000, true);
